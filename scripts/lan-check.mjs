...
let ip = process.env.DASHBOARD_DEV_HOST ?? addresses[0]?.address;
...
try {
  ...
  const portRule = lines.find(
    (l) => /Private|Any/i.test(l.split('|')[0] ?? '') && /\b(3000|5173)\b/.test(l.split('|')[1] ?? ''),
  );
...
for (const [label, port] of [
  ...
  if (!onLoopback) {
    warn(`${label} not running on :${port}`, 'Start it with "npm run dev" and re-run this check.');
  } else if (!onLan) {
    fail(
...
if (ip) {
  ...
  console.log(`  chrome.exe --kiosk --noerrdialogs --disable-infobars http://${ip}:${VITE_PORT}\n`);
}

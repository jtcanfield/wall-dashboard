 ...
 */
function markersPlugin(getLuften: () => LuftenState | null): NonNullable<Options['plugins']>[number] {
      ...
      drawClear: (u: uPlot) => {
        ...
        for (const kind of ['exchange', 'all-day'] as const) {
          ...
          for (const w of windows.filter((x) => x.kind === kind)) {
            ...
            ctx.fillRect(left, u.bbox.top, Math.min(x1, u.bbox.left + u.bbox.width) - left, u.bbox.height);
          }
        }
        ctx.restore();
      },

...
export class WeatherService implements OnModuleInit {
  ...
  private async fetchOpenMeteo(): Promise<WeatherData> {
    ...
    const h = res.hourly;
    if (!h?.time?.length) throw new Error('Open-Meteo returned no hourly series');
    ...
    return { hourly: hourly.filter((x) => !Number.isNaN(x.temperatureF)), source: 'open-meteo' };
  }
  ...
  private async fetchNws(): Promise<WeatherData> {
    ...
    const points = await getJson<NwsPoints>(`https://api.weather.gov/points/${lat},${lon}`, init);
    const hourlyUrl = points.properties?.forecastHourly;
    if (!hourlyUrl) throw new Error('NWS returned no forecastHourly link');

    const res = await getJson<NwsHourly>(hourlyUrl, init);
    const periods = res.properties?.periods ?? [];
    if (periods.length === 0) throw new Error('NWS returned no periods');

    const cutoff = DateTime.now().setZone(TIMEZONE).plus({ days: FORECAST_DAYS }).startOf('day');

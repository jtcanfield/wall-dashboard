...
export class FxService implements OnModuleInit {
  ...
  private async probeRubSupport(): Promise<boolean> {
    if (this.frankfurterHasRub !== null) return this.frankfurterHasRub;
    try {
      ...
      this.log.log(`Frankfurter RUB support: ${this.frankfurterHasRub} (end_date ${rub?.end_date ?? 'none'})`);
    } catch (err) {
      this.log.warn(`Could not probe Frankfurter currencies, falling back to CBR — ${String(err)}`);
      this.frankfurterHasRub = false;
    }
    return this.frankfurterHasRub;
  }

  private async fetchFrankfurter(from: DateTime, quotes: Quote[]): Promise<Partial<Record<Quote, FxPoint[]>>> {
    ...
    for (const row of rows) {
      if (!row.date || !row.quote || typeof row.rate !== 'number') continue;
      if (!wanted.has(row.quote)) continue;
...
function toSeries(quote: Quote, points: FxPoint[], source: FxSeries['source']): FxSeries {
  ...
  return {
    ...
    changePct:
      first && last && first.rate !== 0 ? ((last.rate - first.rate) / first.rate) * 100 : null,
    source,
  };
}

...
export class NewsService implements OnModuleInit {
  ...
  private async fetchAll(): Promise<NewsItem[]> {
    ...
    if (items.length === 0) throw new Error(`all ${failures} feeds produced nothing`);
  ...
  private async toItems(feed: FeedSource, raw: Parser.Item[]): Promise<NewsItem[]> {
    ...
    const kept = raw
      .flatMap((item) => {
        ...
        const parsed = raw ? DateTime.fromISO(DateTime.fromRFC2822(raw).isValid ? DateTime.fromRFC2822(raw).toISO()! : raw) : null;
        ...
        return [{ id: item.guid ?? link, title, link, source: feed.label, publishedAt: publishedAt! }];
      })
    ...
    if (feed.language === 'en' || kept.length === 0) return kept;

...
export function findWindows(
    ...
    for (let i = 0; i < hourly.length; i++) {
        ...
        if (isOpen && openedAt === null) {
            ...
            if (end > openedAt) {
                windows.push({ start: openedAt, end, kind });
            }
            openedAt = null;
        }
    }
    ...
    if (openedAt !== null) {
        ...
        if (last && last.time > openedAt) {
            windows.push({ start: openedAt, end: last.time, kind });
        }
    }
